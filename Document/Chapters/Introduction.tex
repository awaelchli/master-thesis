...
%		\end{equation}
	...
	\section{Recurrent Neural Networks}
		...
		It is a function that maps each vector $\vectr{x}_t \in \mathbb{R}^n$ from the sequence to a hidden state
		\begin{eqnarray}
			\vectr{h}_t = R(\vectr{x}_t, \vectr{h}_{t-1}), & &t = 1, \dots, T
		\end{eqnarray}
		with the hidden state $\vectr{h}_{t-1} \in \mathbb{R}^d$ carried over from the previous prediction as a second input, where $d$ is called the \emph{hidden size} of the RNN. 
		...
		Say we define a criterion $L(\vectr{y}_t, \vectr{\hat{y}}_t)$ for the loss between prediction and ground-truth at each time step $t$.
		Then, the total loss for the sequence is simply 
		\begin{equation}
			L(\{\vectr{y}_1, \dots \vectr{y}_T\}, \{\vectr{\hat{y}}_1, \dots \vectr{\hat{y}}_T\}) = \sum_{t=1}^{T} L(\vectr{y}_t, \vectr{\hat{y}}_t).
		\end{equation}
		The gradient computation is the same as for feed-forward networks and the back-propagation algorithm can be applied by unfolding the RNN as shown in figure~\ref{fig:RNN-unrolled}.
		...
		\end{figure}
		\todo{Transition from RNN to LSTM (gradient vanishing)}
		$R \colon \mathbb{R}^n \times \mathbb{R}^d \times \mathbb{R}^d \rightarrow \mathbb{R}^d \times \mathbb{R}^d$
		\begin{align}\label{eq:Vanilla-LSTM-Definition}
			...
			%\end{split}
		\end{align}
		The matrix $\matr{W} \in \mathbb{R}^{4d \times (n + d)}$ contains the weights for the input- and hidden state transformations before each gate.
		...
		\begin{align}
			...
			\end{bmatrix}
		\end{align}
		\todo{Insert a graphic with all the connections between gates, input, output}
