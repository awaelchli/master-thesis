...
%		
	...
	\section{The Eight Point Algorithm}
		The eight point algorithm, introduced by \cite{longuet1981}, is an algorithm that computes the \emph{essential matrix} $E$ which encodes the relative translation $t$ and rotation $R$ between two cameras as
		\begin{equation}
		...
		When a 3D point is observed by a camera, it projects to a 2D point in the image plane. 
		The second camera observes the same 3D point, but it will project to a different 2D point.
		The epipolar constraint states the the second point, given the first one, must lie on a 2D line (the epipolar line) across the image plane.
		\todo{?? show image from book/paper that shows how epipolar line relates to 3D point}
		In other words, the essential matrix $E$ is the matrix that maps a point $\hat{x}_0$ in image 0 to its epipolar line in $l_1 = E \hat{x}_0$.
		Given a corresponding point $\hat{x}_1$ in image 1, it must lie on the epipolar line, i.e. $\hat{x}_1^\top l_1 = 0$.
