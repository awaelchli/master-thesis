...
%		\end{figure}
	...
	\section{Evaluation on the GTA V Dataset}
		...
		Even though the GTA V dataset is synthetic, it contains many challenging types of motion.
		All motions are captured on foot.
		The average rotation angle between consecutive frames is 
		...
		\begin{table}[tb]
			...
			\caption[Inter-frame motion comparison between the VIPER and GTA V datasets]
					{Inter-frame motion comparison between the VIPER and GTA V datasets.
					 The table shows relative rotation angle and traveled distance between consecutive frames on the test set.
					 \label{tbl:viper-gta-comparison-inter-frame-motion}}
		\end{table}
		
		% gTA
%		Rotation angle between consecutive frames (AVG / MIN / MAX): / 0.0000 /  degrees
%		Distance travelled between consecutive frames (AVG / MIN / MAX):  / 0.0000 /  meters
%		
