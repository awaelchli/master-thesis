...
%	- Dropout Regularization
	...
	\section{Training with Incremental Poses}
		...
		\begin{figure}
			...
			\begin{subfigure}[b]{0.5\linewidth}
				\centering
				\includegraphics[width=\linewidth]{Python-Plots/deepvo-VIPER/rotation-error-per-meter-comparison-LSTM-No-LSTM}
				\caption{
					Rotation
			...
			\begin{subfigure}[b]{0.5\linewidth}
				\centering
				\includegraphics[width=\linewidth]{Python-Plots/deepvo-VIPER/translation-error-per-meter-comparison-LSTM-No-LSTM}
				\caption{
					Translation
					\label{fig:avg-translation-error-VIPER-LSTM-no-LSTM}
				}
			\end{subfigure}%
			\caption[Comparison on VIPER: LSTM vs. no LSTM]
					{Comparing the recurrent and non-recurrent model on the VIPER test dataset.
					 Both version make similar rotation- and translation errors, but overall the recurrent model has worse performance.
					 \label{fig-1}}
		\end{figure}
		...
		\begin{table}[tb]
			...
			\begin{center}
				\begin{tabular}{cccrrrr}
					...
					LSTM & Hidden & & Rotation & Translation & Rotation & Translation \\
					\midrule
					\xmark		& &			& 0.2162	& 7.3836	& 6.7071	& 241.1611		\\
					\cmark 		& & 			& 0.2339	& 5.5290	& 11.4762	& 259.9814		\\
					\cmark		& & 			& & & & \\
					\bottomrule
				\end{tabular}
			\end{center}
			\caption[Experiments on VIPER: Training with incremental poses]
			{Tested on length 100
				\label{tbl:-1}}
		\end{table}
