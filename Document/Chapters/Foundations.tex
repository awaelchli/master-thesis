\chapter{Foundations}\label{chp:foundations}
...
%		In this thesis, we are particularly interested in the subset of quaternions called the \emph{unit quaternions}. These are all the quaternions with unit norm, i.e., $\lVert \vectr{q} \rVert = 1$. 
	...
	\section{Convolutional Neural Networks}
		...
		A layer in a feedforward network can theoretically depend on all the outputs of the previous layer.
		...
		%Furthermore, the number of outputs in $\matr{Y}$ changes according to the input size.
	...
	\section{Training RNNs}
		...
		\todo{Check again! Explicitly refer to the section}
	...
	\section{A First Toy Example: Memorizing Digits}\label{sec:toy-example-memory}
		...
		In order to tackle this machine learning problem, we will define three key components:
		\begin{enumerate}
			\item Model
		...
		\paragraph{Results for Binary Sequences}
		...
		\end{table}
		It is trained and evaluated for each value $C \in \{1, \dots 15\}$.
		\begin{figure}[tb]
			\centering
